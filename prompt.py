...
图片可能包含多张表格（上下排列），请你只读取最上方那张主尺码表的数据，严格忽略下方的"大货洗前尺寸表"或任何标注了"洗前"字样的表格，下方任何表格（洗前）的数据一概忽略，只取上方第一个表格的数据。
...
规则：
- 表格上方有一行合并的单元格是表格名称，只抄写最上方的主（洗后）尺码表，下方任何表格（洗前）的数据一概忽略.只取上方第一个表格的数据。
...
【最重要规则 —— 缺失数据直接填 null】
对于每个映射字段，去第一步抄写结果中查找对应的工厂称呼：
- 找到了 → 填入该行对应的数值
- 找不到 → 该列全部填 null，绝不能从其他行搬运数据来填充
